...
//            "FilewiseUnitTestCode" -> "$storedUrl/assistant/file-testCases"
//            else -> {
        ...
        if (assistantType == "Filewise Unit Test Code") {
            ...
            thread {
                try {
                    ...
                    val jobId = JsonFilewiseUnitTestCode.extractJobID(jobResponse)

                    if (jobId.startsWith("Error") || jobId == "JobID not found") {
                        SwingUtilities.invokeLater {
                            loadingDialog.dispose()
                            JOptionPane.showMessageDialog(null, jobId, "Error", JOptionPane.ERROR_MESSAGE)
                        }
                        return@thread
                    }

                    val statusUrl = "$storedUrl/assistant/file-testCases/status"
                    val results = JsonFilewiseUnitTestCode.pollForResults(statusUrl, jobId, authToken)

                    SwingUtilities.invokeLater {
                        loadingDialog.dispose()
                        if (results.startsWith("Timeout") || results.startsWith("Error")) {
